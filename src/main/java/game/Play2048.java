...
public class Play2048 extends JPanel {
	...
	void drawGrid(Graphics2D g) {
		...
		if (gamestate == State.RUNNING) {
			...
			if (gamestate == State.WON) {
				g.drawString("C'est gagné !", 390, 350);

			} else if (gamestate == State.OVER)
				g.drawString("Game Over !", 400, 350);

			g.setColor(gridColor);
			g.drawString("Cliquez pour débuter une nouvelle partie.", 255, 470);
			g.drawString("(Utilisez les flèches pour bouger les carreaux)", 235, 530);
		}
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			...
			f.setTitle("2048");
			f.setResizable(true);
			f.add(new Play2048(), BorderLayout.CENTER);
			f.pack();
